...
export default class SimpleSFUClient {
    constructor({ port, configuration }) {
        ...
        this.localStream = null;
        this.peers = new Map(); // Store RTCPeerConnection instances
        this.eventListeners = new Map();
    ...
    initSocketListeners() {
        ...
        this.socket.on('answer', ({ to, sdp }) => {
            this.handleAnswer(to, sdp);
        });

        this.socket.on('ice-candidate', ({ to, candidate }) => {
            this.handleIceCandidate(to, candidate);
        });

        this.socket.on('user-joined', ({ userId, username }) => {
            this.trigger('onJoin', { userId, username });
        });

        this.socket.on('user-left', ({ userId }) => {
            this.trigger('onLeave', { userId });
        });
    }
    ...
    async handleOffer(from, sdp) {
        ...
        this.socket.emit('answer', { to: from, sdp: peerConnection.localDescription });
    }

    handleAnswer(to, sdp) {
        const peerConnection = this.peers.get(to);
        const desc = new RTCSessionDescription(sdp);
    ...
    createPeerConnection(userId) {
        ...
        peerConnection.onicecandidate = ({ candidate }) => {
            if (candidate) {
                this.socket.emit('ice-candidate', { to: userId, candidate });
            }
        };

        peerConnection.ontrack = (event) => {
            this.trigger('onRemoteTrack', { stream: event.streams[0], userId });
        };
    ...
    async startStream() {
        ...
        this.trigger('onLocalStream', this.localStream);
    }
    ...
    async callUser(userId) {
        ...
        this.socket.emit('offer', { to: userId, sdp: peerConnection.localDescription });
    }
